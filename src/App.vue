          ...
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 w-full bg-white dark:bg-[#25273C] rounded-md shadow-xl">
              ...
              <ul>
                ...
                <draggable v-model="filterTodos" item-key="id" >
                  <template #item="{element, index}">
                    <li class="grid grid-cols-12 items-center py-3 bg-white dark:bg-[#25273C] border-y dark:border-[#393A4C]" :class="{ 'rounded-t-md' : index === 0 }">
                      ...
                      <p @click="updateTodo(element)" class="col-span-8 text-xs md:text-base cursor-pointer" :class="[element.isDone ? 'line-through text-[#E4E5F1] dark:text-[#4D5066]' : 'text-[#484B6A] dark:text-[#CACDE8]']">{{ element.task }}</p>
